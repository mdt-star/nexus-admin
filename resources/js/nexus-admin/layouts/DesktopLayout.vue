...
// 容器高度缓存：避免每次渲染都读 clientHeight（强制布局重算），减少 ResizeObserver 干扰
let cachedContainerH = 0
function getCachedContainerHeight() {
  if (!cachedContainerH) {
    const container = iconsRef.value
    cachedContainerH = container ? container.clientHeight : window.innerHeight
  }
  return cachedContainerH
}
// 窗口 resize 时刷新缓存
window.addEventListener('resize', () => { cachedContainerH = 0 })
...
function getItemBasePos(item) {
  ...
  const containerH = getCachedContainerHeight()
...
function iconStyle(item) {
  ...
  // 注意：不使用 left/top transition 以避免布局属性动画引发 ResizeObserver 循环
  // 图标位置变化时直接跳转到位，拖拽体验不受影响（拖拽中通过 transform translate 偏移）
  return { position: "absolute", left: p.x + "px", top: p.y + "px", zIndex: lastDragItem.value === item.id ? 998 : 1 }
}
