  ...
  <div class="nexus-sidebar-layout">
    ...
    >
      ...
      <div class="nexus-sidebar-logo" :class="{ 'nexus-sidebar-logo-themed': hasHeaderColor }" :style="hasHeaderColor ? headerStyle : {}">
        <div class="nexus-sidebar-logo-inner" :class="{ 'nexus-sidebar-logo-collapsed': appStore.sidebarCollapsed }">
          <el-icon class="nexus-sidebar-logo-icon" :size="logoIconSize"><TrendCharts /></el-icon>
      ...
      <header class="nexus-header" :style="headerStyle">
        ...
        <div class="nexus-header-right">
          ...
          <el-dropdown @command="(val) => uiSizeStore.setSize(val)">
            <el-button><span style="font-size:13px;font-weight:600">{{ uiSizeStore.size.charAt(0).toUpperCase() }}</span></el-button>
            <template #dropdown>
              <el-dropdown-menu>
          ...
          <el-dropdown @command="handleLocaleChange">
            <el-button><globe-icon style="vertical-align: middle;" /> {{ localeDisplay }} <el-icon class="el-icon--right"><ArrowDown /></el-icon></el-button>
            <template #dropdown>
              <el-dropdown-menu>
    ...
    <Teleport to="body">
      <div v-if="sidebarContextVisible" class="nexus-context-menu" :style="sidebarContextStyle" @click.stop>
        <div class="nexus-context-item" @click="editSidebarItem"><el-icon><Edit /></el-icon><span>编辑</span></div>
        <div class="nexus-context-item" @click="deleteSidebarItem"><el-icon><Delete /></el-icon><span>删除</span></div>
        <div class="nexus-context-divider" />
        <div class="nexus-context-item" @click="addSidebarFolder"><el-icon><FolderAdd /></el-icon><span>新建文件夹</span></div>
      </div>
    </Teleport>
  </div>
</template>
...
import { ArrowLeft, ArrowRight, TrendCharts, Plus, Edit, Delete, FolderAdd } from '@element-plus/icons-vue'
...
function toggleStartMenu() { startMenuVisible.value = !startMenuVisible.value }
...
.nexus-sidebar { width: var(--nexus-sidebar-width); background-color: var(--nexus-bg-color-light); border-right: 1px solid var(--nexus-border-color); display: flex; flex-direction: column; transition: width 0.3s; overflow: hidden; position: relative; }
...
.nexus-sidebar-dragover { position: relative; outline: 2px dashed var(--nexus-primary-color); outline-offset: -2px; background-color: color-mix(in srgb, var(--nexus-primary-color) 6%, transparent); }
.nexus-sidebar-drop-overlay { position: absolute; inset: 0; z-index: 100; display: flex; align-items: center; justify-content: center; pointer-events: none; }
.nexus-sidebar-drop-hint { display: flex; flex-direction: column; align-items: center; gap: 8px; color: var(--nexus-primary-color); font-size: 14px; font-weight: 500; animation: nexus-drop-pulse 1.2s ease-in-out infinite; }
@keyframes nexus-drop-pulse { 0%, 100% { transform: scale(1); opacity: 0.8; } 50% { transform: scale(1.05); opacity: 1; } }
...
.nexus-sidebar-collapsed .nexus-sidebar-menu :deep(.el-sub-menu__title) { padding: 0 !important; display: flex; align-items: center; justify-content: center; }
.nexus-sidebar-collapsed .nexus-sidebar-menu :deep(.el-menu-item > div),
.nexus-sidebar-collapsed .nexus-sidebar-menu :deep(.el-sub-menu__title > div) { padding: 0 !important; display: flex; align-items: center; justify-content: center; }
...
.nexus-header[style*="background"] .nexus-header-right :deep(.el-button), .nexus-header[style*="background"] .nexus-header-left :deep(.el-button) { color: rgba(255,255,255,0.9) !important; }
.nexus-header[style*="background"] .nexus-header-right :deep(.el-button:hover), .nexus-header[style*="background"] .nexus-header-left :deep(.el-button:hover) { background-color: rgba(255,255,255,0.15) !important; color: #fff !important; }
...
.nexus-header-right :deep(.el-button), .nexus-header-left :deep(.el-button) { border: none; background-color: transparent; color: var(--nexus-text-color-secondary); transition: all 0.25s ease; margin-left: 0 !important; }
.nexus-header-right :deep(.el-button:hover), .nexus-header-left :deep(.el-button:hover) { background-color: var(--nexus-bg-color-dark); color: var(--nexus-primary-color); }
...
.nexus-tab { margin-top: 0px; display: flex; align-items: center; gap: 6px; padding: 0 14px 0 28px; font-size: var(--nexus-font-size-sm); cursor: pointer; white-space: nowrap; user-select: none; position: relative; color: var(--nexus-text-color-secondary); transition: all 0.2s ease; border-right: 1px solid var(--nexus-border-color); }
...
.nexus-tabs-actions { display: flex; align-items: stretch; gap: 0; }
.nexus-tabs-actions .nexus-tab-btn { border: none !important; border-radius: 0 !important; background-color: transparent; color: var(--nexus-text-color-secondary); height: auto; width: 28px; padding: 0; margin-left: 0 !important; border-right: 1px solid var(--nexus-border-color) !important; }
.nexus-tabs-actions .nexus-tab-btn:last-child { border-right: none !important; }
.nexus-tabs-actions .nexus-tab-btn:hover { background-color: var(--nexus-bg-color-light); color: var(--nexus-primary-color); }
.nexus-tabs-actions .nexus-tab-btn:disabled { color: var(--nexus-text-color-disabled, #bbb) !important; cursor: default; opacity: 0.6; }
.nexus-tabs-actions .nexus-tab-btn:disabled:hover { background-color: transparent; color: var(--nexus-text-color-disabled, #bbb) !important; }
