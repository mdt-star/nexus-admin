...
describe('MenuStore', () => {
  ...
  it('loadMenus() 从 API 加载菜单', async () => {
    ...
    menusApi.list.mockResolvedValue({ data: mockData })
  ...
  it('loadMenus() API 失败时使用空数组', async () => {
    const { default: menusApi } = await import('../services/menus')
    menusApi.list.mockRejectedValue(new Error('Network error'))
  ...
  it('loadMenus() 重复调用不重复加载', async () => {
    const { default: menusApi } = await import('../services/menus')
    menusApi.list.mockResolvedValue({ data: [] })
    ...
    await store.loadMenus()
    expect(menusApi.list).toHaveBeenCalledTimes(2)
  })
